...
Compatible Databricks (notebook + wheel task) et exécution locale
"""
...
def _get_spark_session():
    ...
    if is_databricks:
        ...
        # STRATÉGIE 1 : Essayer de récupérer la session active
        spark = SparkSession.getActiveSession()
        
        if spark is not None:
            print("   ✅ Session Spark active récupérée")
            return spark
        
        # STRATÉGIE 2 : Essayer d'accéder à la variable globale 'spark' (notebooks)
        try:
            import __main__
            if hasattr(__main__, 'spark'):
                spark = __main__.spark
                print("   ✅ Session Spark globale récupérée (notebook)")
                return spark
        except Exception:
            pass
        
        # STRATÉGIE 3 : Créer une session avec config Databricks
        print("   ⚙️  Création d'une nouvelle session Spark...")
        try:
            ...
            print(f"   ❌ Impossible de créer session Spark: {e}")
            raise RuntimeError(
                "Impossible d'obtenir une session Spark sur Databricks.\n"
                "Vérifiez que le cluster est correctement configuré."
            )
...
def after_all(context):
    ...
    # Sur Databricks, ne pas arrêter Spark si elle existait déjà
